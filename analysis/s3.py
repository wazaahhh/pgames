...
def uploadJsonViz(rootDir,filename):
    ...
    keyname = "%s_%s.json"%(filename[:-4],token)
    key = bucketviz.new_key(keyname)
...
def makeKey(filename):
    descDic = parseFilename(filename)
    token = str(uuid.uuid4())[-5:]
    key = "results/%s_%s.json.zlib"%(filename[:-4],token)
    return key
